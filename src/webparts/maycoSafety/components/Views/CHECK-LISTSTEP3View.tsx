...
export default class CheckList3View extends React.Component<ActionsProps,ActionsState> {
  ...
  public componentDidMount() {
    document.title = "Mayco - Safety | CHECK-LIST STEP 3 View";
    this.loadListData();
  }
    ...
    const columns = [
                    ...
                    return (
                      <React.Fragment>
                        <div style={{ paddingLeft: '10px' }}>
                          <NavLink title="Edit" className="csrLink ms-draggable" to={`/CHECK-LISTSTEP3Form/${record.Id}`}>
      ...
      const filteredData =this.state.selectedYear && this.state.selectedYear !== "All" ? this.state.ActionsData.filter( (item) => item.Year === this.state.selectedYear ): this.state.ActionsData;
       if(this.state.redirect){
                    let url = `/CHECK-LISTSTEP3Form/${this.state.ItemID}`;
