  ...
  private async loadListData() {
    try {
      ...
      const tableData = items.map((item: any) => ({
        ...
        ShiftType: [null, undefined, ''].includes(item.ShiftType) ? item.ShiftType : Number(item.ShiftType),
        ...
        CompletedDate: [null, undefined, ''].includes(item.CompletedDate) ? item.CompletedDate : DateUtilities.getDateMMDDYYYY(item.CompletedDate),
        CompletedDateForGrid: [null, undefined, ''].includes(item.CompletedDate) ? item.CompletedDate : `<span class='d-none'>${DateUtilities.getDateYYYYMMDDForSorting(item.CompletedDate)}</span>${DateUtilities.getDateMMDDYYYY(item.CompletedDate)}`,
  ...
  private handleConfirmDelete = async (id?: number | null) => {
    try {
      if (id != null) {
        ...
        // Delete parent
        await this.sp.web.lists.getByTitle("WCC").items.getById(parentId).delete().then(res => {
          showToast("success", `SMAT - ${parentId}  deleted Successfully`);
          this.setState(prev => ({
  ...
  public render() {
    const columns = [
      ...
      { name: "ID", selector: (row: any) => row.Id, sortable: true, width: '60px' },
      {
        ...
        cell: (row: any) => <div className='' dangerouslySetInnerHTML={{ __html: row.WCCDateForGrid }} onClick={(event) => this.handleRowClicked(event, row.Id)} />,
        sortable: true
      },
      ...
      {
        name: "Supervisor Action Completed",
        selector: (row: any) => row.CompletedDateForGrid,
        cell: (row: any) => <div className='' dangerouslySetInnerHTML={{ __html: row.CompletedDateForGrid }} onClick={(event) => this.handleRowClicked(event, row.Id)} />,
        sortable: true
      },
    ];
